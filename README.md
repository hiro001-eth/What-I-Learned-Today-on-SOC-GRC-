...
* Targets the human layer, bypassing perimeter network security controls.
* Leverages user trust in recognized services (Microsoft 365, Google Workspace, DocuSign, SharePoint).
* Phishing-as-a-Service (PaaS) platforms democratize custom infrastructure deployment.
* Widespread misconfigurations in email authentication (SPF, DKIM, DMARC).
* Adversary-in-the-Middle (AiTM) proxy frameworks effectively bypass standard Multi-Factor Authentication (MFA).
...
| **Nation-State (APT)** | Advanced | Custom redirect networks, compromised compromised cloud tenants, zero-day lures | APT29, Lazarus Group, APT28 |
...
Below is a annotated sample of an unparsed raw email header:
...
### 7.2 Container Format Evadence (ISO, VHD, Password-Protected ZIP)
...
| Threat Actor Group | Target Sectors | Primary Lure Topics | Preferred Delivery Payload / Technique | Key Evading Mechanism |
| :-- | :-- | :-- | :-- | :-- |
...
| | MXToolbox Header Analyzer | Quick visual breakdown of SPF/DKIM/DMARC headers |
| **Reputation Check** | VirusTotal | Multi-engine file, hash, IP, and domain inspection |
| | AbuseIPDB | Community-driven IP abuse and spam reports |
| | URLscan.io | Sandbox URL scanner with full DOM analysis and screenshots |
| **Sandbox & Payload** | ANY.run | Interactive Windows malware execution environment |
| | Hybrid Analysis | Free automated sandbox analysis powered by Falcon Sandbox |
| **Command Utilities** | `zbar-tools` | Command line QR code reading tool (`zbarimg`) |
| | `oletools` | Python suite for parsing MS Office files and macros (`olevba`) |
